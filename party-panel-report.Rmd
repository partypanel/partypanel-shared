...
```{r participant-recruitment-over-time, results='asis'}

knitFig(plotToDraw=participantRecruitmentOverTime,
        figWidth = min(2*c(defaultFigWidth, maxFigWidth)),
        figHeight = min(1.5*c(defaultFigHeight, maxFigHeight)),
...
for (currentFreqVar in names(frequencyVarNames)) {
  ...
  if (length(frequencyVarNames[[currentFreqVar]]) == 0) {
    ...
    pandoc.header("Frequencies and easybarplots", 4);
    
    multiVarFreq(dat, items=frequencyCategoryVarNames$goingOutFreq, sortByMean=FALSE);

    ggEasyBar(dat, items=frequencyCategoryVarNames$goingOutFreq, sortByMean=FALSE);
...
```{r CIBERplot-explanation, results='asis', message=FALSE}
...
The title doubles as a legend: each higher-level determinants (called a 'target' in CIBER plots) is designated a color that is used in the plot. In this case, attitude is red and intention is blue. These colors are used in the right-hand panel of the plot (see below).
...
```{r ciber-plot-generation, screenshot.force=TRUE}
...
```{r generic-infographic, results="asis"}
...
pander(freq(dat$education));

if (!is.null(participantOrigins)) {
