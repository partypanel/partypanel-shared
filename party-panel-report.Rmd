...
singleQuestionEasyBar <- function(item,
  ...
  if (knitFig) {
    cat("\n\n");
    pandoc.p(paste0("These are the results for question (or rather, variable) '", item, "'."));
    cat("\n\n");
  }
  
  if ((item %in% labelDf$varNames.cln) && (nchar(trim(questionText)) > 0)) {
    cat("\n\n");
    pandoc.p(paste0("The lead-in for this question was <str>", questionText, "</str>."));
    cat("\n\n");
  }
...
### Full disclosure, confidentiality and embargo
...
Als je dit interessant vindt, staan er ‘Open Access’ (gratis toegankelijke) inleidende, en verdiepende, artikelen over gedragsverandering op https://effectivebehaviorchange.com
...
`r cat(waveSpecificRandomNumberStatement); `
...
for (i in seq_along(detStruct)) {
  detStruct[[i]]$Do(function(currentNode) {
    ...
    if ((!is.null(currentNode$scaleVarName)) &&
      ...
      if (is.null(currentNode$scaleVarName)) {
        ...
        singleQuestionEasyBarList[[length(singleQuestionEasyBarList)+1]]  <<- 
          #arrangeGrob(singleQuestionEasyBar(currentNode$scaleVarName,
          #                                  knitFig=FALSE),
    ...
    if (!is.null(currentNode$determinantImportance)) {
      ...
      ggSave(plot=currentNode$determinantImportance,
